...
fun indexingCatalogue(catalogue: Resource): JsonObject = JsonObject().apply {
    if (catalogue.isURIResource && DCATAPUriSchema.parseUriRef(catalogue.uri) != null) {
...
fun indexingDataset(dataset: Resource, catalogueId: String? = null, defaultLang: String): JsonObject = JsonObject().apply {
    if (dataset.isURIResource && DCATAPUriSchema.parseUriRef(dataset.uri) != null) {
...
//                DCTerms.language -> {}
//                DCTerms.conformsTo -> {}
                DCAT.mediaType -> putIfNotNull("media_type", obj.visitWith(StandardVisitor)) // to be implemented
//                DCTerms.issued -> {}
//                DCTerms.rights -> {}
//                ADMS.status -> {}
                DCTerms.title -> withJsonObject("title").putPair(indexingMultiLang(obj, defaultLang))
...
fun indexingPublisher(node: RDFNode): JsonObject = when (node) {
    is Resource -> JsonObject().apply {
        CorporateBodies.getConcept(node)?.let {
            ...
            putIfNotNull("email", it.resource.getPropertyResourceValue(FOAF.homepage)?.uri)
